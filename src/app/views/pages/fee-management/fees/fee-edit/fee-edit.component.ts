...
export class FeeEditComponent implements OnInit, OnDestroy {
	...
	initProduct() {
		...
		this.subheaderService.setTitle('Edit fee');
		this.subheaderService.setBreadcrumbs([
			{ title: 'Fee Management', page: `/fee-management` },
			{ title: 'Fees', page: `/fee-management/fees` },
			{ title: 'Edit fee', page: `/fee-management/fees/edit`, queryParams: { id: this.product.id } }
		]);
	}
	...
	updateProduct(_product: ProductModel, withBack: boolean = false) {
		...
		of(undefined).pipe(delay(3000)).subscribe(() => { // Remove this line
			if (withBack) {
				...
				const message = `Fee successfully has been saved.`;
	...
	getComponentTitle() {
		...
		result = `Edit fee - ${this.product.manufacture} ${this.product.model}, ${this.product.modelYear}`;
		return result;
	}
