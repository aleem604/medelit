...
@Component({
	...
	selector: 'kt-professional-service-fee-dialog',
	templateUrl: './professional-service-fee.dialog.component.html',
	styleUrls: ['./professional-service-fee.dialog.component.scss'],
	changeDetection: ChangeDetectionStrategy.OnPush,
	encapsulation: ViewEncapsulation.None
})
export class ProfessionalServiceFeeDialogComponent implements OnInit, OnDestroy {
	serviceModel: ServiceModel;
	feeDiagModel: FeeDialogModel = new FeeDialogModel();
	...
	constructor(public dialogRef: MatDialogRef<ProfessionalServiceFeeDialogComponent>,
		@Inject(MAT_DIALOG_DATA) public data: ServiceModel,
		private serviceslService: ServicesService,
	...
	loadDialogData() {
		...
		this.serviceslService.getServiceById(this.serviceModel.id).toPromise().then((resp) => {
			var res = resp as unknown as ApiResponse;
			if (res.success) {
				this.feeDiagModel.ptFeeId = res.data.ptFeeId;
				this.feeDiagModel.proFeeId = res.data.proFeeId;
	...
	onSumbit(withBack: boolean = false) {
		...
		this.viewLoading = true;
		this.serviceslService.addUpdateFeeToService(this.prepareFee()).toPromise().then((res) => {
			if (res.success) {
	...
	prepareFee(): FeeDialogModel {
		...
		_fee.serviceId = this.serviceModel.id;
	...
	loadPTFeesForFilter() {
		this.staticService.getPTFeesForFilter().subscribe(res => {
			...
			if (this.feeDiagModel.ptFeeId > 0) {
				var vat = this.ptFeesForFilter.find(x => x.id == this.feeDiagModel.ptFeeId);
				if (vat) {
					// @ts-ignore
					this.feeForm.patchValue({ 'ptFeeId': { id: vat.id, value: vat.value, a1: vat.a1, a2: vat.a2 } });
				}
			}
		});
	}
	...
	loadPROFeesForFilter() {
		this.staticService.getPROFeesForFilter().subscribe(res => {
			...
			if (this.feeDiagModel.proFeeId > 0) {
				var vat = this.proFeesForFilter.find(x => x.id == this.feeDiagModel.proFeeId);
				if (vat) {
					// @ts-ignore
					this.feeForm.patchValue({ 'proFeeId': { id: vat.id, value: vat.value, a1: vat.a1, a2: vat.a2 } });
				}
