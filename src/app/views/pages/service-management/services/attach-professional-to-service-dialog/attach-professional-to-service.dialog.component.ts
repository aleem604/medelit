...
@Component({
	...
	selector: 'attach-professional-to-service-dialog',
	templateUrl: './attach-professional-to-service.dialog.component.html',
	styleUrls: ['./attach-professional-to-service.dialog.component.scss']
})
export class AttachProfessionalToServiceDialogComponent implements OnInit, OnDestroy {
	proService: ProfessionalModel;
	@ViewChild(MatSort, { static: true }) sort: MatSort;
	@ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
	viewLoading = false;

	displayedColumns: string[] = ['select', 'id', 'name', 'field', 'subCategory', 'tags'];
	dataSource = new MatTableDataSource<ProfessionalModel>();
	...
	constructor(public dialogRef: MatDialogRef<AttachProfessionalToServiceDialogComponent>,
	...
	loadDialogData() {
		...
		this.servicesService.getProfessionalsToConnectWithService(this.data).toPromise().then((resp) => {
			var res = resp as unknown as ApiResponse;
			if (res.success) {
				this.dataSource = new MatTableDataSource<ProfessionalModel>(res.data);
	...
	checkboxLabel(row?: ProfessionalModel): string {
		if (!row) {
	...
	save() {
		...
		this.viewLoading = true;
		this.servicesService.attachProfessionalsToService(this.proService.id).toPromise().then((res) => {
			if (res.success) {
	...
	loadServicesDataForFilter() {
		this.servicesService.getProfessionalsToConnectWithService(this.data).subscribe(res => {
