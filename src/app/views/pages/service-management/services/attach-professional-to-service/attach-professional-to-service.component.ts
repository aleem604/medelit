...
import { ApiResponse, ProfessionalConnectedServicesModel, ProfessionalsService, ServiceModel, ServicesService } from '../../../../../core/medelit';
import { NgxSpinnerService } from 'ngx-spinner';
import { LayoutUtilsService, MessageType } from '../../../../../core/_base/crud';

import { ConfirmDialogComponent } from '../../../../partials/confirm-dialog/confirm-dialog.component';
import { AttachProfessionalToServiceDialogComponent } from '../attach-professional-to-service-dialog/attach-professional-to-service.dialog.component';
import { AddProfessionalToServiceDialogComponent } from '../add-professional-to-service-dialog/add-professional-to-service.dialog.component';

@Component({
	// tslint:disable-next-line:component-selector
	selector: 'attach-professional-to-service',
	templateUrl: './attach-professional-to-service.component.html',
	styleUrls: ['./attach-professional-to-service.component.scss'],
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class AttachProfessionalToServiceComponent implements OnInit, OnDestroy {
	@Input("proId") proId;
	...
	loadGridData() {
		...
		this.spinner.show();
		this.servicesService.getProfessionalsToConnectWithService(this.proId).toPromise().then((resp) => {
	...
	connectService() {
		const dialogRef = this.dialog.open(AttachProfessionalToServiceDialogComponent, { data: this.proId });
	...
	detachServices() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.spinner.show();
			this.servicesService.detachProfessionalConnectedServices(serviceIds, this.proId).toPromise()
				.then((res) => {
					this.loadGridData();
	...
	removeService(serviceId: number) {
		...
		dialogRef.afterClosed().subscribe(result => {
			if (result) {
				this.spinner.show();
				this.servicesService.detachProfessionalConnectedServices([serviceId], this.proId).toPromise().then((res) => {
					if (res.success) {
						this.loadGridData();
	...
	editServiceFee(serviceRow: ProfessionalConnectedServicesModel) {
		const dialogRef = this.dialog.open(AddProfessionalToServiceDialogComponent, { data: serviceRow });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
